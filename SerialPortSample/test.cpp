...
#include "include/sl_lidar.h"          //雷达头文件
#include "include/sl_lidar_driver.h"   //雷达驱动头文件
#ifndef _countof
#define _countof(_Array) (int)(sizeof(_Array) / sizeof(_Array[0]))
#endif

#ifdef _WIN32        //Windows上运行
#include <Windows.h>
#define delay(x)   ::Sleep(x)
#else
#include <unistd.h>  //POSIX上运行
...
void print_usage(int argc, const char* argv[])
{
    printf("Usage:\n"
        ...
        " C1 Lidar only support 460800 baudrate\n"
...
int main(int argc, const char* argv[]) {
    ...
    sl_u32         baudrateArray = 406800;
    sl_result     op_result;
    int          opt_channel_type = CHANNEL_TYPE_SERIALPORT;//0x0
    ...
    {
        if (!opt_channel_param_first) {//未输入端口则使用默认端口
#ifdef _WIN32
            // use default com port
            opt_channel_param_first = "com8";
#elif __APPLE__
    ...
    if (opt_channel_type == CHANNEL_TYPE_SERIALPORT) {
        if (useArgcBaudrate) {
            ...
            size_t baudRateArraySize = (sizeof(baudrateArray)) / (sizeof(baudrateArray));
            for (size_t i = 0; i < baudRateArraySize; ++i)
            {
                _channel = (*createSerialPortChannel(opt_channel_param_first, baudrateArray));
                if (SL_IS_OK((drv)->connect(_channel))) {
                    op_result = drv->getDeviceInfo(devinfo);
